...
namespace TraCuuSach
{
    public partial class FormTraCuu : Form
    {
        ...
        public void export(DataGridView g, string DuongDan, string TenTep)
        {
        ...
        public ComboBox CbTenSach
        {
            get { return cbTenSach; }
            set { cbTenSach = value; }
        }
        ...
        public DataGridView DgvDanhSachCuonSach
        {
            get { return dgvDanhSachCuonSach; }
            set { dgvDanhSachCuonSach = value; }
        }
        ...
        void LoadDataGridView()
        {
            ...
            {
                if (dgvDanhSachCuonSach.Rows[i].Cells[5].Value.ToString() == "0")
                {
        ...
        public void but2_Click(object sender, EventArgs e)
        {
            ...
            if (s1 != "")
            {
                ...
                search.SetSearchStrategy(new SearchByMSStrategy());
                table.Clear();
                s = s1;
            ...
            if (s2 != "")
            {
                ...
                //command.ExecuteNonQuery();
                search.SetSearchStrategy(new SearchByTypeStrategy());
                table.Clear();
                s = s2;
            }
            if (s3 != "")
            {
                ...
                //command.ExecuteNonQuery();
                search.SetSearchStrategy(new SearchByNameStrategy());
                table.Clear();
                s = s3;
            }
            if (s4 != "")
            {
                ...
                //command.ExecuteNonQuery();
                search.SetSearchStrategy(new SearchByAuthorStrategy());
                table.Clear();
                s = s4;
            }
            if (s5 != "")
            {
                ...
                search.SetSearchStrategy(new SearchByStatusStrategy());
                table.Clear();
                s = s5;
            }
            ...
            if (s1 != "" || s3 != "" || s2 != "" || s4 != "" || s5 != "")
            {
                ...
                table = search.GetSearchStrategyResult(s);
                //adapter.Fill(table);
        ...
        {
            SqlConnection connection = new SqlConnection(LibraryManagement.Models.DatabaseInfo.connectionString);
        ...
        public void cbTenSach_KeyPress(object sender, KeyPressEventArgs e)
        {
            this.AutoComplete(this.cbTenSach, e, true);
        }

        public void cbTacGia_KeyPress(object sender, KeyPressEventArgs e)
        {
            this.AutoComplete(this.cbTacGia, e, true);
        }

        public void cbTinhTrang_KeyPress(object sender, KeyPressEventArgs e)
        {
        }
